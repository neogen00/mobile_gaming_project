...
from settings import (DB_HOST, DB_USER, DB_NAME, DB_PASSWORD)
...
def get_db():
    if "db" not in g:
        g.db = psycopg2.connect(user = current_app.config['DB_USER'], password = current_app.config['DB_PASSWORD'], host = current_app.config['DB_HOST'],
